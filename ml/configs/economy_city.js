...
export default {
  ...
  sampleRide() {
    ...
    surge *= 1 + Math.pow(Math.max(demand_ratio - 1, 0), 1.1) * 0.10;
    ...
    else if (hour >= 17 && hour <= 20) surge += 0.30;
    else if (hour >= 20 && hour <= 23) surge += 0.11;
    else if (hour >= 0 && hour <= 5)   surge += 0.13;
    else                                surge -= 0.05;

    // Weather — car reversal: rain RAISES surge
    if (weather_code === 2)      { surge += 0.07; demand_ratio *= 1.10; }
    else if (weather_code === 3) { surge += 0.15; demand_ratio *= 1.25; }
    else if (weather_code === 4) { surge += 0.26; demand_ratio *= 1.5; }
    else if (weather_code === 5) { surge += 0.32; demand_ratio *= 1.7; }
    else if (weather_code === 6) { surge += 0.15; }
    else if (weather_code === 7) { surge += 0.11; }
