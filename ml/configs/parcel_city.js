...
export default {
  ...
  sampleRide() {
    ...
    if (hour >= 10 && hour <= 14)      surge += 0.14; // daytime commerce peak
    else if (hour >= 18 && hour <= 21) surge += 0.18; // evening orders
    else if (hour >= 0 && hour <= 6)   surge -= 0.08; // dead hours, cheap
    else                                surge += 0.02;

    // Weather — INSENSITIVE for demand; only a small supply-side bump in severe
    if (weather_code === 5) surge += 0.08; // storm slows riders
