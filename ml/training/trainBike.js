...
async function train() {
        ...
        callbacks: {
            onEpochEnd: (epoch, logs) => {
                if (epoch % 5 === 0 || epoch === 39)
                    console.log(
                        `Epoch ${epoch}: loss=${logs.loss.toFixed(5)}, val_mae=${logs.val_mae.toFixed(4)}`
                    );
            }
        }
    });
    ...
    testScenario("Clear, 8am rush, high demand", [
        ...
        3.5,
        8,
        Math.sin((2 * Math.PI * 8) / 24),
        Math.cos((2 * Math.PI * 8) / 24),
        Math.sin((2 * Math.PI * 1) / 7),
        Math.cos((2 * Math.PI * 1) / 7),
        0,
        0,
        0
    ]);
    ...
    testScenario("3am, clear, low demand (should be LOW)", [
        ...
        0.8,
        25,
        Math.sin((2 * Math.PI * 3) / 24),
        Math.cos((2 * Math.PI * 3) / 24),
