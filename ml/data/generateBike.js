...
function generateBikeRide() {
    ...
    else if (traffic_ratio >= 1.35) surge += 0.1;
    else if (traffic_ratio >= 1.15) surge += 0.05;

    // --- 3. Time of day (additive, small) ---
    if (hour >= 7 && hour <= 9)
        surge += 0.15; // morning rush
    else if (hour >= 17 && hour <= 20)
        surge += 0.2; // evening rush
    else if (hour >= 20 && hour <= 23)
        surge += 0.08; // night
    else if (hour >= 0 && hour <= 5)
        surge += 0.12; // late night
    ...
    surge = Math.max(0.8, Math.min(surge, BIKE.max_surge));
    surge = round(surge, 3);
