...
export function sampleIntercityRide(p) {
    ...
    const cancellation_risk = round(Math.min(0.6, (booking_lead_time_hours / 168) * 0.5 + rand(0, 0.1)), 3);
    ...
    const wsev = (w) => (w === 5 ? 0.18 : w === 4 ? 0.1 : w === 6 ? 0.08 : w === 3 ? 0.05 : 0);
    ...
    if (m === 5 || m === 6 || m === 11)
        surge += 0.12; // Jun/Jul summer, Dec break
    ...
    return {
        ...
        traffic_ratio,
        avg_speed_kmh,
        weather_code: origin.weather_code,
        rain_mm: origin.rain_mm,
        visibility_m: origin.visibility_m,
        wind_speed: origin.wind_speed,
        feels_like_temp: origin.feels_like_temp,
        dest_weather_code: dest.weather_code,
        dest_rain_mm: dest.rain_mm,
        ...
        is_public_holiday: time.is_public_holiday,
        is_ramadan: time.is_ramadan,
        cancellation_risk,
