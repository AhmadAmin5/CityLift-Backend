...
export function buildFeatureRow(rec, featureNames) {
    return featureNames.map((name) => {
        switch (name) {
            case "hour_sin":
                return Math.sin((2 * Math.PI * rec.hour) / 24);
            case "hour_cos":
                return Math.cos((2 * Math.PI * rec.hour) / 24);
            case "day_sin":
                return Math.sin((2 * Math.PI * rec.day) / 7);
            case "day_cos":
                return Math.cos((2 * Math.PI * rec.day) / 7);
            case "month_sin":
                return Math.sin((2 * Math.PI * rec.month) / 12);
            case "month_cos":
                return Math.cos((2 * Math.PI * rec.month) / 12);
            default:
                return rec[name];
        }
    });
